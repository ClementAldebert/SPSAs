...
# -*- coding: utf-8 -*-
"""
Created on Thu Jul 12 14:29:55 2018
...
# Modified the 12/07/2018 to save different   #
...
class SPSA():
    ...
    def __init__(self,Y,cost_func,tol,n_iter,gamma,alpha,A,comp=False,
        ...
        self.n_anim=n_anim # To determine the frequency of refreshing of the
        # Animation
    ...
    def SPSA_adaptative_step_size(self,mult_size):
        ...
        while self.J[-1] > self.tol and self.k_grad < self.n_iter :
            ...
            grad_k_1=grad_k.copy() # Saving the last estimate
            # Plotting the animation
            if self.anim :
                if self.k_grad%self.n_anim==0 : 
                    self.Anim_func() # Call the function for animation
    ...
    def SPSA_accelerating_step_size(self,mult_size,max_acc=5,min_acc=0.01):
        """ A version of SPSA estimation where we compare the angle between
        two evaluation of the gradient to get the step size of the next modi-
        fication of the parameters vector. We use a normal gain sequence ak
        that we multiply by a factor mult_size^cos(theta), where theta designate 
...
#            self.update_c()
    ...
    def SPSA_mean_accelerating_step_size(self,n_mean,mult_size,max_acc=5,min_acc=0.01):
        """ A version of SPSA estimation where we compare the angle between
        two evaluation of the gradient to get the step size of the next modi-
        fication of the parameters vector. We use a normal gain sequence ak
        that we multiply by a factor mult_size^cos(theta), where theta designate 
...
#            self.update_c()
            ...
            # Gradient estimation (from mean SPSA method)
            for i in range(n_mean):
                if i==0:
                    grad_k=self.grad()
                else :
    ...
    def SPSA_accelerating_step_size_momentum(self,mult_size,max_acc=5,min_acc=0.01,mom_coeff=.9):
        """ A version of SPSA estimation where we compare the angle between
        two evaluation of the gradient to get the step size of the next modi-
        fication of the parameters vector. We use a normal gain sequence ak
        that we multiply by a factor mult_size^cos(theta), where theta designate 
...
#            self.update_c()
            ...
            if self.k_grad==1:
               ...
               print('Ich bin in Accelerating-SPSA !')
                ...
                if acc > max_acc: acc=max_acc
                if acc < min_acc: acc=min_acc
                grad_k=(grad_k+mom_coeff*grad_k_1)/(1.+mom_coeff)    # add a momentum
    ...
    def SPSA_mean_accelerating_step_size_momentum(self,n_mean,mult_size,max_acc=5,min_acc=0.01,mom_coeff=.9):
        """ A version of SPSA estimation where we compare the angle between
        two evaluation of the gradient to get the step size of the next modi-
        fication of the parameters vector. We use a normal gain sequence ak
        that we multiply by a factor mult_size^cos(theta), where theta designate 
...
#            self.update_c()
    ...
    def SPSA_stochastic_RMS_prop(self,batch_size,mom_coeff=0.9):
            """ A version of SPSA estimation where we search the gradient on 
            subspace and add it to the previously obtained gradients. The goal 
            is to break the isotropy of SPSA and make it able to solve not so 
            well conditionned problems.
...
#                    self.dir_mat*=inv_v
                    ...
                    # We average only when the estimation is not null
#                    grad_estim=self.grad()
                    for i in range(len(grad_estim)):
                        if grad_k[i]!=0.:
                            grad_estim[i]=mom_coeff*grad_estim.copy()[i]\
                                +(1.-mom_coeff)*grad_k[i]
                print('Hessian',v)
                print('Gradient',grad_estim)
                print('Position',self.Y)
                print('Increment for gradient',self.dir_mat*self.c)
