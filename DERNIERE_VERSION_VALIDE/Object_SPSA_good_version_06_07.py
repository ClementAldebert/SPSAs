    ...
    def SPSA_BFGS(self):
        ...
        while self.J[-1] > self.tol and self.k_grad < self.n_iter :
            ...
            # Then we can use the BFGS for the next step
                ...
                u=np.outer(self.y,self.y)/np.dot(self.y,-self.a@self.grad_estim)
                # Then we use another matrix proportionnal to the product of 
                # The last gradient estimation. @ designate a scalar product
                # Normalize we do not use the direction vector but the direction
                # Vector multiplied by the step size. But since the step size 
                # Is a scalar present on both numerator and denominator, we can
                # cancel it
                v=-np.outer(-self.B@self.grad_estim,-self.B@self.grad_estim)/(-self.p@(self.B@self.grad_estim))
                # We can update b
                self.B+=u+v
